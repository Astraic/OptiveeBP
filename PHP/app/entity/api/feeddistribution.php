...
require_once(dirname(__FILE__,3) . '/entity/database/FeedDistribution.php');

/**
 * API ter behoeve van framework voor feeddistribution model class.
 * @author Stephan de Jongh
 */

class FeedDistribution extends \app\framework\api\ForcedApi {
    ...
    public function bindModel(\app\framework\model\Model $model, Array $value) : \app\framework\model\Model {
          switch($value[0]){
            ...
            case 'allocated':
                ...
                break;     
            case 'consumed':
                $model->setConsumed(end($value));
                break;          
          }
        return $model;
    }
}
$api = new \app\entity\api\FeedDistribution();
?>