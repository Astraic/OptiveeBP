...
require_once(dirname(__FILE__,3) . '/entity/model/MeatGrade.php');

/**
 * Databinding voor create, update en read queries voor meatgrade model class.
 * @author Stephan de Jongh
 */

class MeatGrade extends \framework\database\CRUD implements \framework\database\CRUInterface {

    // Constructor ter voorbereiding prepared insert statement.
    function __construct(QueryBuilderParent ...$query){
        $sql = "INSERT INTO MeatGrade (name) VALUES (:name)";
    ...
    function select(\model\Model $model) : array{
        ...
        $results = $this->select[0]->fetchAll(\PDO::FETCH_CLASS|\PDO::FETCH_PROPS_LATE, 'model\MeatGrade');
